...
// extract form data from each message and write it to a pdf at the end
...
class Winlink_Checkins
{
    ...
    public static void Main (string [] args)
    {
        try
        {
            ...
            // Console.WriteLine ("This is to make sure I am executing the right program");
            ...
            DateTime netStartDate = startDate.Date;
            // If startDate is not Monday, move forward to the next Monday
            int daysToAdd = ((int)DayOfWeek.Monday - (int)netStartDate.DayOfWeek + 7) % 7;
            netStartDate = netStartDate.AddDays (daysToAdd);
            ...
            if (!string.IsNullOrWhiteSpace (yourCallSign))
            {
            ...
            string? netName = "";
            // Look for roster.txt in the folder. 
            string rosterFile = dataFolder + "\\roster.txt";
            // string attachmentFileCSV = dataFolder + "\\attachments.csv";

            string xmlFile = dataFolder + "\\Winlink_Import.xml"; // separate file for defective messages
            string xmlPerfFile = dataFolder + "\\Winlink_Import_Perfect.xml"; // separate file for perfect messages
                                                                              // string commentFile = dataFolder+"\\"+ netName +"_Additional_Comments.txt";
            ...
            if (File.Exists (rosterFile))
            {
                ...
                if (startPosition > -1)
                {
                    ...
                    endPosition = rosterString.IndexOf ("//", startPosition);
                    len = endPosition - startPosition;
                    if (len > 0) { credentialFilename = rosterString.Substring (startPosition, len).Trim (); } else credentialFilename = "credentials.json";
                }
                else
                {
                    Console.WriteLine ("The Google credentials filename is missing from the roster.txt file. \r\nA default will be tried. If not found, the spreadsheet will not be updated.");
            ...
            {
                //Console.WriteLine (dataFolder + "\\" + rosterFile + " \n was not found! A new one will be created. \r\n"
            ...
            var files = Directory.GetFiles (dataFolder, "*.mime")
                .Where (file =>
                {
            ...
            using (StreamWriter logWrite = new (Path.Combine (dataFolder, "checkins.txt")))
            // Create a csv file called checkins.csv in the data folder and process the list of files.
            using (StreamWriter csvWrite = new (Path.Combine (dataFolder, "checkins.csv")))
            ...
            using (StreamWriter mapWrite = new (Path.Combine (dataFolder, "mapfile.csv")))
            // Create a text file called Additional Comments.txt in the data folder 
            using (StreamWriter commentWrite = new (Path.Combine (dataFolder, netName + " Additional Comments.txt")))
            // Create a csv file called attachments.csv in the data folder and process the list of files.
            using (StreamWriter attachmentCSVwrite = new (Path.Combine (dataFolder, "attachments.csv")))

            {
                ...
                foreach (string file in files)
                {
                    using (StreamReader reader = new StreamReader (file))
                    {
                            ...
                            if (startPosition > -1)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (attachmentDecoded.Length > 0) // Removed redundant != null check
                                    {
                                        ...
                                        string outputFilePath = dataFolder + "\\attachment.csv";
                        ...
                        if (forwarded == -1) forwarded = fileText.IndexOf ("WINLINK HAS AUTOMATICALLY FORWARDED THIS MESSAGE");
                        ...
                        // get From:
                        if (forwarded > -1) startPosition = forwarded; // this assumes that the forwarded message is not also from APRS
                        ...
                        var ICS213 = fileText.IndexOf ("TEMPLATE VERSION: ICS 213 ");
                        if (ICS213 > -1) ICS213Ct++;
                        ...
                        if (startPR > -1) ack = -1;
                        if (fileText.Contains ("LATITUDE: ") && fileText.Contains ("LONGITUDE: ") && ICS213 == -1 && winlinkCkin == -1)
                        {
                        ...
                        // check for ICS-214A Individual Activity Log
                        var ICS214A = fileText.IndexOf ("ICS214A");
                        if (ICS214A > -1) ICS214ACt++;

                        // check for ICS-214 Activity Log
                        if (ICS214A == -1) ICS214 = fileText.IndexOf ("ICS214"); // only check if ICS214A not found to avoid false positive
                        if (ICS214 > -1) ICS214Ct++;

                        // check for ICS-215A Incident Safety Analysis
                        var ICS215A = fileText.IndexOf ("ICS215A");
                        if (ICS215A > -1) ICS215ACt++;

                        // check for ICS-217A Incident Safety Analysis
                        var ICS217A = fileText.IndexOf ("ICS 217A");
                        if (ICS217A > -1) ICS217ACt++;

                        //Check for Request for Information Strip
                        RFIstrip = fileText.IndexOf ("RESPONSE STRIP:"); // the RFI strip message includes the original request so look for the "RESPONSE STRIP:" label
                        if (RFIstrip > -1) RFIstripCt++;
                        ...
                        else
                        {
                            ...
                            if (fileText.Contains (netName!))
                            {
                                ...
                                if (startPosition > -1)
                                {
                                    endPosition = fileText.IndexOf ("\r\n", startPosition + 2);
                                    len = endPosition - startPosition;
                                    // screen out those that sent the example data instead of their own
                                    examplePosition = fileText.IndexOf ("XXNXXX", startPosition, len);
                                    if (examplePosition == -1) examplePosition = fileText.IndexOf ("FIRSTNAME", startPosition, len);
                                    // if (examplePosition == -1) examplePosition = fileText.IndexOf ("GL4WN", startPosition, len);
                                ...
                                if (startPosition == -1) newFormat = false;
                                else
                                {
                                    // endPosition = startPosition;
                                    if (exampleIncluded) // remove the example if it is included to avoid junk
                                    {
                                        // startPosition = examplePosition;
                                        // startPosition = fileText.LastIndexOf ("##", startPosition);
                                        // endPosition = fileText.IndexOf ("##", startPosition + 2);
                                        // if (endPosition == -1) endPosition = fileText.IndexOf ("\r\n", startPosition) + 2;
                                        //  len = endPosition - startPosition;
                                        if (len > 0) fileText = fileText.Remove (startPosition, len);
                                        startPosition = fileText.IndexOf ("##");
                                    }
                                    // if the message is Request for Information strip, skip down to the response strip which will always have a start ##
                                    if (RFIstrip > -1)
                                    {
                                        startPosition = fileText.IndexOf ("##", RFIstrip);
                                        if (startPosition == -1) startPosition = endPosition; // not a real GLAWN RFI strip message for some reason
                                    }
                                ...
                                else // this sections finds the msgField location within the checkin data
                                {
                                    ...
                                    else if (ics213rr > -1 && !newFormat) // otherwise there will be a whole bunch of possible delimiters
                                    {
                                        startPosition = fileText.IndexOf ("17. REPLY");
                                        startPosition = fileText.IndexOf ("\r\n", startPosition) + 2;
                                        endPosition = fileText.IndexOf ("18. FINANCE SECTION NAME:", startPosition) - 1;
                                    }
                                    else if (ICS214 > -1 && !newFormat) // If it is not the new format and it is an ICS214, the checkin data can't be found
                                    {
                                        startPosition = -1;
                                    }
                                    else if (ICS214A > -1 && !newFormat) // If it is not the new format and it is an ICS214A, the checkin data can't be found
                                    {
                                        startPosition = -1;
                                    }
                                ...
                                if (startPosition == -1 && ((ICS214 > -1) || (ICS214A > -1))) // If it is not the new format and it is an ICS214 or A, the checkin data can't be found
                                {
                                    if (quotedPrintable > -1) startPosition = quotedPrintable;
                                    else startPosition = endHeader;
                                ...
                                if (startPosition > -1)
                                {
                                    ...
                                    originalMsgField = fileText.Substring (startPosition, endPosition - startPosition);
                                    ...
                                    checkinItems = null; // empty the array
                                                         // if it is an RFI Strip, there shouldn't be any '|' or "," characters. If there are, the standard format was pasted
                                                         // into the calculated field instead and this is an attempt to recover.
                                    if (RFIstrip > -1) msgField = msgField.Replace ("|", "/")
                                            .Replace ("//", "/")
                                            .Replace (",", " ");
                                    len = msgField.Length;
                                    if (len > 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            Console.WriteLine ("1475 Invalid checkin data in messageID: " + safeMessageID);
                                            isPerfect = false;
                                            score = 1;
                                        }
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (len > 0)
                                        {
                                            if (checkinItems [0] != checkIn) checkinItems [0] = checkIn;
                                            if (len < 8)
                                            {
                                                score = score - (8 - len);
                                            ...
                                            if (checkinItems != null && len >= 7)
                                            {
                                                ...
                                                bandStr = item6 != null ? item6.Trim ().Trim (',').Replace ("5G", "5CM") : "";
                                                ...
                                                if (bandStr == "")
                                                {
                                                    ...
                                                    {
                                                        pointsOff += "\tAREDN is a project, not a valid band or mode. Try \"5CM, 9CM, 13CM, 33CM, or SHF\" or \"MESH\"\r\n";
                                                    }
                                                }
                                                else
                                                {
                                            ...
                                            if (checkinItems != null && len >= 8)
                                            {
                                                ...
                                                {
                                                    (modeStr, bandStr, modeTypo) = checkMode (modeStr ?? string.Empty, bandStr ?? string.Empty, modeTypo ?? string.Empty);
                                                    if (modeTypo != string.Empty)
                                                    {
                                                        reminderTxt += modeTypo;
                                                    }
                                                    if (modeStr != checkinItems [7]) checkinItems [7] = modeStr;
                                                }
                                                else
                                                {
                                                ...
                                                if (bandStr != "" && bandStr != checkinItems [6]) checkinItems [6] = bandStr;
                                            }
                                    ...
                                    else if (RFIstrip > -1 && checkinItems != null && checkinItems.Length > 9 && checkinItems [9] != null)
                                    {
                                        addonString.Append (checkIn + ":\t" + checkinItems [9] + "\r\n");
                                    }
                                    else
                                    {
                                    ...
                                    if (latitudeStr == "" || longitudeStr == "")
                                    {
                                        ...
                                        if (len > 0)
                                        {
                                            if (ExtractCoordinates (fileText.Substring (startPosition, len), out latitude, out longitude))
                                            {
                                                ...
                                                if (maidenheadGrid == "invalid") Console.WriteLine (messageID); else mapCt++;
                                            }
                                            else
                                            {
                ...
                if (!string.IsNullOrWhiteSpace (spreadsheetId))
                {
                    ...
                    if (newCheckIns != null && tmpInput != "Y" && tmpInput != "S")
                    {
                        ...
                        // Normalize startDate to the Monday ON OR AFTER it
                        // DateTime netStartDate = startDate.Date; - declared earlier
                        // while (netStartDate.DayOfWeek != DayOfWeek.Monday) this is duplicate code 
                        //    netStartDate = netStartDate.AddDays (1);
                ...
                if (ICS213Ct > 0) { logWrite.WriteLine ("ICS-213 Checkins: " + ICS213Ct); }
                ...
                if (ICS201Ct > 0) { logWrite.WriteLine ("ICS201 Checkins: " + ICS201Ct); }
                if (ICS202Ct > 0) { logWrite.WriteLine ("ICS202 Checkins: " + ICS202Ct); }
                if (ICS203Ct > 0) { logWrite.WriteLine ("ICS203 Checkins: " + ICS203Ct); }
                if (ICS204Ct > 0) { logWrite.WriteLine ("ICS204 Checkins: " + ICS204Ct); }
                if (ICS205Ct > 0) { logWrite.WriteLine ("ICS205 Checkins: " + ICS205Ct); }
                if (ICS205aCt > 0) { logWrite.WriteLine ("ICS205a Checkins: " + ICS205aCt); }
                if (ICS206Ct > 0) { logWrite.WriteLine ("ICS206 Checkins: " + ICS206Ct); }
                if (ICS208Ct > 0) { logWrite.WriteLine ("ICS208 Checkins: " + ICS208Ct); }
                if (ICS210Ct > 0) { logWrite.WriteLine ("ICS210 Checkins: " + ICS210Ct); }
                if (ics213RRCt > 0) { logWrite.WriteLine ("ICS213RR Checkins: " + ics213RRCt); }
                if (ICS214Ct > 0) { logWrite.WriteLine ("ICS214 Checkins: " + ICS214Ct); }
                if (ICS214ACt > 0) { logWrite.WriteLine ("ICS214A Checkins: " + ICS214ACt); }
                if (ICS215ACt > 0) { logWrite.WriteLine ("ICS215A Checkins: " + ICS215ACt); }
                if (ICS217ACt > 0) { logWrite.WriteLine ("ICS217A Checkins: " + ICS217ACt); }
                if (RFIstripCt > 0) { logWrite.WriteLine ("RFI Strip Checkins: " + RFIstripCt); }
                ...
                logWrite.WriteLine ("Total Plain and other Checkins: " + (ct - localWeatherCt - severeWeatherCt - incidentStatusCt - ICS213Ct - winlinkCkinCt - damAssessCt - fieldSitCt - qwmCt - dyfiCt - rriCt - qwmCt - miCt - aprsCt - meshCt - PosRepCt - ICS201Ct - radioGramCt - ICS202Ct - ICS203Ct - ICS204Ct - ICS205Ct - ICS205aCt - ICS206Ct - ICS208Ct - ICS210Ct - ics213RRCt - WBBMct - ICS214Ct - ICS214ACt - RFIstripCt - ICS215ACt - ICS217ACt - skipped) + "\r\n");
                ...
                // Call python script to extract forms from the messages
                string exeDir = Path.GetDirectoryName (System.Reflection.Assembly.GetExecutingAssembly ().Location)!;
                string pythonScript = Path.Combine (exeDir, "extractWinlinkFormData.py");
                string mimeDir = exeDir;  // Or specific subfolder: Path.Combine(exeDir, "IncomingMimes")
                ...
                if (File.Exists (pythonScript))
                {
                    ...
                    if (process != null)
                    {
                        process.WaitForExit ();  // Wait for Python to finish (blocks C#)
                ...
                {
                    Console.WriteLine ("extractWinlinkFormData.py not found!");
                }
        ...
        catch (Exception ex)
        {
            Console.Error.WriteLine (
                "Winlink Checkins requires .NET 8 Desktop Runtime.\n" +
                "If this is a new machine, please install .NET 8 from:\n" +
    ...
    public static void SortStringBuilder (StringBuilder sb, string delimiter, int rowsToSkip)
    {
        ...
        if (rowsToSkip > 0)
        {
            header = string.Join (delimiter, items.Take (rowsToSkip));
            if (header.Length > 0)
                header += delimiter;
        }
        ...
        // Join everything and CLEAN IT
        string sortedContent = (header + string.Join (delimiter, rowsToSort))
            .Trim (delimiter.ToCharArray ())
            .Replace ("\r\n\r\n", "\r\n");
    ...
    public static string checkBand (string input)
    {
        ...
        // Check for obvious band names FIRST (prevents false positives)
        ...
        if (IsKnownBand (normalized))
            return normalized;

        // NEW: Next, try to extract and normalize ANY numeric frequency to MHz
        Match numMatch = Regex.Match (input, @"(\d+(?:\.\d+)?)");
        if (numMatch.Success)
        {
            if (double.TryParse (numMatch.Groups [1].Value, out double rawVal))
            {
                double freqMHz;
                if (rawVal >= 10000.0) // e.g., 7102.00 kHz -> 7.102 MHz
                    freqMHz = rawVal / 1000.0;
                else if (rawVal > 1000.0) // e.g., 5330.5 kHz -> 5.3305 MHz
                    freqMHz = rawVal / 1000.0;
                else // Already MHz, e.g., 7.102, 144.390
                    freqMHz = rawVal;

                // Map to standard band name [web:13][web:15]
                if (freqMHz >= 1.8 && freqMHz < 2.0) return "160M";
                else if (freqMHz >= 3.5 && freqMHz < 4.0) return "80M";
                else if (freqMHz >= 5.3 && freqMHz < 5.4) return "60M";
                ...
                else if (freqMHz >= 50.0 && freqMHz < 54.0) return "6M";
                else if (freqMHz >= 144.0 && freqMHz < 148.0) return "2M";
                else if (freqMHz >= 222.0 && freqMHz < 225.0) return "1.25M";
                else if (freqMHz >= 420.0 && freqMHz < 450.0) return "70CM";
                else if (freqMHz >= 500.0) return "SHF";
            }
        }

        // FALLBACK: Original text normalization for band names
        input = input
            .Replace ("METERS", "M")
            .Replace ("MTRS", "M")
            .Replace ("MTR", "M")
    ...
    //static (DateTime, DateTime, string) getNetDates (DateTime startDate, DateTime endDate, string weekDay, int netLength)
    {
        ...
        switch (input)
        {
            ...
            case "VARA":
                ...
                error = "\r\nTypo found in the mode: " + input + " - normalized to VARA HF or VARA FM based on the band: " + input2;
                break;
            ...
            case "SPMT":
            case "STMP":
                error = "\r\nTypo in the mode field: " + input + " - normalized to SMTP";
                input = "SMTP";
                break;
            ...
            case "VARA FN":
            case "VARAFM":
                error = "\r\nTypo found in the mode: " + input + " - normalized to VARA FM";
                input = "VARA FM";
                break;
            ...
            case "VARA-SSB":
                if (input2 == "")
                {
                    input2 = "HF"; // if band is blank but mode indicates HF, set band to HF
                    error = "\r\nBand missing - normalized to HF based on the mode";
                }
                error += "\r\nTypo found in the mode: " + input + " - normalized to VARA HF";
                input = "VARA HF";
                break;
            ...
            default:
                ...
                input = "";
                error = "\r\nNo valid mode found for: " + input;
                break;
        }
        return (input, input2, error);
    }
    ...
    public static string getMsgField (int startPosition, int endPosition, string messageID, string fileText, string? msgField)
    {
        ...
        // the ICS217A does not have a single field large enough for the checkin data so it will be 
        // spread across multiple lines. The not wanted info will be separated by tabs
        var ICS217A = fileText.IndexOf ("ICS 217A");
    ...
    public static string? [] getCheckinData (int len, string? msgField, string? []? checkinItems, bool newFormat)
    {
        ...
        // Clean the input
        string checkIn = msgField.Replace ("(", "")
            .Replace (")", "")
            .Replace ("/STANDARD FORMS,GENERAL FORMS,REQUEST FOR INFORMATION STRIP", "\r\nSTANDARD FORMS,GENERAL FORMS,REQUEST FOR INFORMATION STRIP");
        // remove extra lines
        if (checkIn.IndexOf ("\r\n") != -1)
        {
            len = checkIn.IndexOf ("\r\n");
    ...
    // -----------------------------------------
    // Method to update Google Sheet with check-in data
    private static void UpdateGoogleSheet (StringBuilder netCheckinString, StringBuilder netAckString2, StringBuilder newCheckins, StringBuilder removalString, string spreadsheetId, DateTime endDate, string credentialFilename)
    {
        try
        {
            // string credentialsPath = Path.Combine (Directory.GetCurrentDirectory (), credentialFilename);
            string exeDir = Path.GetDirectoryName (Assembly.GetExecutingAssembly ().Location)!;
            string credentialsPath = Path.Combine (exeDir, credentialFilename);
            // string credentialsPath = Path.Combine (Environment.CurrentDirectory, credentialFilename);
            ...
            //var service = new SheetsService (new BaseClientService.Initializer - this has been deprecated by Google, 2025
            //{
            ...
            GoogleCredential credential = CredentialFactory
                .FromFile (credentialFilename, "service_account")   // JSON service-account file
            ...
            //This section has a bug that only manifests when the ISO week Monday is out of alignment - occurred first on 12/8/2025
            ...
            // Compute ISO week Monday and week number

            // This should keep the weeks in alignment
            // ------------------------------------------------------------
            // Determine reporting week and column for the net
            // ------------------------------------------------------------

            // Treat Sunday nets as belonging to the following week
            DateTime reportingDate = endDate.Date;
            if (reportingDate.DayOfWeek == DayOfWeek.Sunday)
            {
                reportingDate = reportingDate.AddDays (1);
            }

            // Find the Monday of that reporting week
            DateTime monday = reportingDate;
            int daysToSubtract = (7 + (monday.DayOfWeek - DayOfWeek.Monday)) % 7;
            monday = monday.AddDays (-daysToSubtract);

            // Find the first Monday of the year
            DateTime firstMonday = new DateTime (monday.Year, 1, 1);
            ...
            int weekNumber = ((int)(monday - firstMonday).TotalDays / 7) + 1;
            if (weekNumber < 1) weekNumber = 1;
            if (weekNumber > 53) weekNumber = 53;

            // Map week number to spreadsheet column (Week 1 = O, Week 52 = BO)
            string columnLetter = GetColumnLetter (weekNumber);

            // DEBUG: print computed week and column
            Console.WriteLine ($"DEBUG: endDate={endDate:yyyy-MM-dd}, reportingDate={reportingDate:yyyy-MM-dd}, monday={monday:yyyy-MM-dd}");
            Console.WriteLine ($"DEBUG: weekNumber={weekNumber}, columnLetter={columnLetter}, mapped column index={GetColumnIndex (columnLetter)}");

            // Optional: print all weeks in the year to verify column mapping
            //for (int w = 1; w <= 53; w++)
            //{
            //    string col = GetColumnLetter (w);
            //    Console.WriteLine ($"Week {w,2} -> Column {col}");
            //}
    ...
    {
        int columnNumber = weekNumber + (14); // Week 1 = O (15th column), Week 53 = BO (67th), physical column count starts at 1, the column function is 0 based.
    ...
    private static bool IsKnownBand (string s)
    {
        return s == "160M" || s == "80M" || s == "60M" || s == "40M" || s == "30M" ||
               s == "20M" || s == "17M" || s == "15M" || s == "12M" || s == "10M" ||
               s == "6M" || s == "2M" || s == "1.25M" || s == "70CM" || s == "TELNET";
    }
}